...
MailPage::MailPage(const QUrl &url, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MailPage)
{
    ui->setupUi(this);
    ...
    toolsMenu->addAction(tr("Remove object elements"), this, SLOT(removeObjectElements()));
...
void MailPage::adjustLocation()
{
    locationEdit->setText(view->url().toString());
}

void MailPage::changeLocation()
{
...
void MailPage::setProgress(int p)
{
    progress = p;
    adjustTitle();
}

void MailPage::finishLoading(bool)
{
    progress = 100;
    adjustTitle();
...
void MailPage::removeGifImages()
{
    QString code = "qt.jQuery('[src*=gif]').remove()";
    view->page()->runJavaScript(code);
}

void MailPage::removeInlineFrames()
{
    QString code = "qt.jQuery('iframe').remove()";
    view->page()->runJavaScript(code);
}

void MailPage::removeObjectElements()
{
    QString code = "qt.jQuery('object').remove()";
    view->page()->runJavaScript(code);
}

void MailPage::removeEmbeddedElements()
{
    QString code = "qt.jQuery('embed').remove()";
    view->page()->runJavaScript(code);
}

void MailPage::renewPage()
{
    view->reload();
}
